...
- Broader test coverage: the remaining admin CRUD controllers (offices, initiative questions, events, attendance, question library, grades) don't have dedicated tests yet — `InitiativeControllerTest`/`ConfigSetControllerTest`/`VolunteerAdminControllerTest` establish the `@WebMvcTest` pattern to extend to them
